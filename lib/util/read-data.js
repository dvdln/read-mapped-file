  ...
  fs.exists(options.src, function(exists) {
    if (exists) {
      fs.readFile(options.src, options.fs, function(err, content) {
        ...
        data[options.resultKey] = parseContent(content, options.json, errors);
...
function readSync(data, options) {
  if (fs.existsSync(options.src)) {
    ...
    data[options.resultKey] = parseContent(content, options.json);
  }

  return data;
}

function parseContent(content, json, errors) {
